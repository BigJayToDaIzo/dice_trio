...
pub fn roll_d6_statistical_distribution_test() {
  let results = list.range(1, 100)
  |> list.map(fn(_) {
    case dice_trio.roll("d6", fn(max) { 
...
pub fn roll_with_modifier_range_validation_test() {
  let test_cases = [
    #("d6+10", 11, 16),  // d6(1-6) + 10 = 11-16
    #("2d6+3", 5, 15),   // 2d6(2-12) + 3 = 5-15  
    #("d20-5", -4, 15),  // d20(1-20) - 5 = -4-15
  ]
...
pub fn roll_large_dice_expression_test() {
  ...
  result |> should.equal(Ok(5050))  // 100 × 50 + 50 = 5050
}

pub fn roll_extreme_boundary_conditions_test() {
  let fixed_rng = fn(max) { max }  // Always roll maximum
  ...
  |> should.equal(Ok(6000))  // 1000 × 6 = 6000
}

pub fn roll_negative_results_test() {
  let fixed_rng = fn(_) { 1 }  // Always minimum roll
  
  dice_trio.roll("d6-10", fixed_rng)
  |> should.equal(Ok(-9))  // 1 - 10 = -9
  
  dice_trio.roll("2d6-20", fixed_rng) 
  |> should.equal(Ok(-18))  // (1+1) - 20 = -18
}
